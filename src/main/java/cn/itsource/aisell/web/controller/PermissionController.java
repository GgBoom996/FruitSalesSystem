...
public class PermissionController {
    ...
    @ResponseBody
    public UIPage<Permission> datagrid(PermissionQuery query){
        Page<Permission> pageByQuery = permissionService.findPageByQuery(query);
        return new UIPage<Permission>(pageByQuery);
    }
    ...
    @ModelAttribute("updatePermission")//在执行目标方法之前，它会首先执行当前方法
    public Permission beforeEdit(Long id){
        if(id != null){
    ...
    /**
     * 添加和修改都是此方法
     * @param permission  判断id是否为空，如果id为空，则新增，否则修改
     * @return
     */
    @RequestMapping("/save")
    @ResponseBody
    ...
    @ResponseBody
    public AjaxResult update(@ModelAttribute("updatePermission") Permission permission){
        return saveOrUpdate(permission);
    }
